...
#       endif
        ...
        LOG_INFO("%s " WHITE_BOLD("HIZ-") "SANiYE:10/SANiYE:60/DAKiKA:15-" CYAN_BOLD("%s") CYAN(" %s %s ") CYAN_BOLD("%s") " MAKSiMUM: " CYAN_BOLD("%s %s") "%s",
                 Tags::miner(),
...
void xmrig::Miner::setEnabled(bool enabled)
{
    ...
    if (d_ptr->controller->config()->isPauseOnBattery() && d_ptr->battery_power && enabled) {
        LOG_INFO("%s " YELLOW_BOLD("PiL GüCüYLE ÇALIŞIRKEN DEVAM EDEMiYOR"), Tags::miner());
    ...
    if (enabled) {
        LOG_INFO("%s " GREEN_BOLD("DEVAM ETTiRiLDi"), Tags::miner());
    }
    else {
        if (d_ptr->battery_power) {
            LOG_INFO("%s " YELLOW_BOLD("DURAKLATILDI"), Tags::miner());
        }
        else {
            LOG_INFO("%s " YELLOW_BOLD("DURAKLATILDI") ", DEVAM ETMEK iÇiN " MAGENTA_BG_BOLD(" R ") " TUŞUNA BASINIZ", Tags::miner());
        }
    }
...
void xmrig::Miner::onTimer(const Timer *)
{
    ...
    if (config->isPauseOnBattery()) {
        autoPause(d_ptr->battery_power, Platform::isOnBatteryPower(), YELLOW_BOLD("PiL GüCüNDE"), GREEN_BOLD("AC GüCüNDE"));
    }

    if (config->isPauseOnActive()) {
        autoPause(d_ptr->user_active, Platform::isUserActive(config->idleTime()), YELLOW_BOLD("KULLANICI AKTiF"), GREEN_BOLD("KULLANICI ETKiN DEĞiL"));
    }
