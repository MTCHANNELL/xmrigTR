...
#   endif
    ...
    LOG_NOTICE("%s " WHITE_BOLD("KIYASLAMA TAMAMLANDI ") CYAN_BOLD("%.3f seconds (%.1f h/s)") WHITE_BOLD_S " hash sum = " CLEAR "%s%016" PRIX64 CLEAR, tag(), dt, BenchState::size() / dt, color, result);
...
{
    LOG_INFO("%s " WHITE_BOLD("ÇIKMAK iÇiN ") MAGENTA_BOLD("CTRL+C") WHITE_BOLD(" TUŞUNA BASINIZ"), tag());
}
...
void xmrig::BenchClient::start()
{
    ...
    LOG_NOTICE("%s " MAGENTA_BOLD("KIYASLAMA BAŞLAT ") "KARMALAR: " CYAN_BOLD("%u%s") " ALGORiTMA: " WHITE_BOLD("%s"),
               tag(),
...
{
    LOG_NOTICE("%s " WHITE_BOLD("KIYASLAMA GöNDERiLDi ") CYAN_BOLD("https://xmrig.com/benchmark/%s"), tag(), m_job.id().data());
    printExit();
}
...
{
    LOG_ERR("%s " RED("%s: ") RED_BOLD("\"%s\""), tag(), label ? label : "KIYASLAMA BAŞARISIZ", message);
    printExit();
