...
inline static void printCount(uint64_t accepted, uint64_t rejected)
{
    ...
    Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-17s") CSI "1;3%dm%" PRIu64 CLEAR CSI "0;3%dm (%1.1f%%)", "KABUL EDiLDi         :", color, accepted, color, percent);

    if (rejected) {
        Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-17s") RED_BOLD("%" PRIu64), "REDDEDiLMiŞ          :", rejected);
    }
}
...
{
    Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-17s") CYAN_BOLD("%" PRIu64) "/ORTALAMA: " CYAN("%1.0f"),
               "HAVUZ KARMALARI      :", hashes, static_cast<double>(hashes) / accepted);
}


inline static void printAvgTime(uint64_t time)
{
    Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-17s") CSI "1;3%dm%1.1fs", "ORTALAMA SONUÇ SüRESi:", (time < 10000 ? 3 : 2), time / 1000.0);
}


static void printDiff(uint64_t diff)
{
    Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-17s") CYAN_BOLD("%s"), "ZORLUK               :", NetworkState::humanDiff(diff).c_str());
}
...
inline static void printLatency(uint32_t latency)
{
    ...
    Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-17s") CSI "1;3%dm%ums", "PiNG ZAMANI    :", color, latency);
}
...
{
    if (!m_active) {
        LOG_NOTICE(YELLOW_BOLD_S "AKTiF BAĞLANTI YOK");

        return;
    }

    Log::print(MAGENTA_BOLD_S " - BAĞLANTI");
    Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-17s") CYAN_BOLD("%s ") BLACK_BOLD("(%s) ") GREEN_BOLD("%s"),
               "HAVUZ ADRESi   :", m_pool, m_ip.data(), m_tls.isNull() ? "" : m_tls.data());

    Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-17s") WHITE_BOLD("%s"), "ALGORiTMA      :", m_algorithm.name());
    printDiff(m_diff);
    printLatency(latency());
    Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-17s") CYAN_BOLD("%" PRIu64 "s"), "BAĞLANTI ZAMANI: ", connectionTime() / 1000);
}
...
{
    if (!m_hashes) {
        LOG_NOTICE(YELLOW_BOLD_S "HENüZ SONUÇ YOK");

        return;
    }

    Log::print(MAGENTA_BOLD_S " - SONUÇLAR");
    ...
    Log::print(WHITE_BOLD_S "  # |     ZORLUK |   ÇABA %% |");
