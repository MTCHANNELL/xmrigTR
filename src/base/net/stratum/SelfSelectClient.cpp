...
bool xmrig::SelfSelectClient::parseResponse(int64_t id, rapidjson::Value &result, const rapidjson::Value &error)
{
    ...
    if (error.IsObject()) {
        LOG_ERR("[%s] HATA: " RED_BOLD("\"%s\"") RED_S ", KOD: %d", pool().daemon().url().data(), Json::getString(error, "message"), Json::getInt(error, "code"));
...
void xmrig::SelfSelectClient::submitBlockTemplate(rapidjson::Value &result)
{
    ...
    send(doc, [this](const rapidjson::Value &result, bool success, uint64_t) {
        if (!success) {
            if (!isQuiet()) {
                LOG_ERR("[%s] HATA: " RED_BOLD("\"%s\"") RED_S ", KOD: %d", pool().daemon().url().data(), Json::getString(result, "message"), Json::getInt(result, "code"));
            }
