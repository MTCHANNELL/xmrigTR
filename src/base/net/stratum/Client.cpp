...
void xmrig::Client::onResolved(const DnsRecords &records, int status, const char *error)
{
    ...
    if (status < 0 && records.isEmpty()) {
        if (!isQuiet()) {
            LOG_ERR("%s " RED("DNS HATASI: ") RED_BOLD("\"%s\""), tag(), error);
        }
...
bool xmrig::Client::verifyAlgorithm(const Algorithm &algorithm, const char *algo) const
{
    if (!algorithm.isValid()) {
        if (!isQuiet()) {
            if (algo == nullptr) {
                LOG_ERR("%s " RED("BiLiNMEYEN ALGORiTMA, AYARLADIĞINIZDAN EMiN OLUN: \"ALGORiTMA\" VEYA \"MADENi PARA\" AYARLA"), tag(), algo);
            }
            else {
                LOG_ERR("%s " RED("BiLiNMEYEN ALGORiTMA ") RED_BOLD("\"%s\" ") RED("ALGILANDI, YENiDEN BAĞLAN"), tag(), algo);
            }
        }
...
bool xmrig::Client::write(const uv_buf_t &buf)
{
    ...
    if (!isQuiet()) {
        LOG_ERR("%s " RED("YAZMA HATASI: ") RED_BOLD("\"%s\""), tag(), uv_strerror(rc));
    }
...
void xmrig::Client::parse(char *line, size_t len)
{
    ...
    if (error.IsObject()) {
        if (!isQuiet()) {
            LOG_ERR("%s " RED("HATA: ") RED_BOLD("\"%s\"") RED(", KOD: ") RED_BOLD("%d"),
...
void xmrig::Client::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    ...
    if (error.IsObject()) {
        ...
        if (!handleSubmitResponse(id, message) && !isQuiet()) {
            LOG_ERR("%s " RED("HATA: ") RED_BOLD("\"%s\"") RED(", KOD: ") RED_BOLD("%d"), tag(), message, Json::getInt(error, "code"));
        }
...
void xmrig::Client::read(ssize_t nread, const uv_buf_t *buf)
{
    ...
    if (nread < 0) {
        if (!isQuiet()) {
            LOG_ERR("%s " RED("OKUMA HATASI: ") RED_BOLD("\"%s\""), tag(), uv_strerror(static_cast<int>(nread)));
        }
...
void xmrig::Client::onConnect(uv_connect_t *req, int status)
{
    ...
    if (status < 0) {
        if (!client->isQuiet()) {
            LOG_ERR("%s " RED("BAĞLANTI HATASI: ") RED_BOLD("\"%s\""), client->tag(), uv_strerror(status));
        }
