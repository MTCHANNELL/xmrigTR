...
bool xmrig::RxMsr::init(const RxConfig &config, const std::vector<CpuThread> &threads)
{
    ...
    if (m_cacheQoS && !Cpu::info()->hasCatL3()) {
        LOG_WARN("%s " YELLOW_BOLD("BU iŞLEMCi CAT-L3 DESTEKLEMiYOR, ÖNBELLEK QOS KULLANILAMIYOR"), Msr::tag());

        m_cacheQoS = false;
    }

    if ((m_enabled = wrmsr(preset, threads, m_cacheQoS, config.rdmsr()))) {
        LOG_NOTICE("%s " GREEN_BOLD("KAYIT DEĞERLERi \"%s\" ÖN AYARI iÇiN BAŞARIYLA AYARLANDI") BLACK_BOLD(" (%" PRIu64 " ms)"), Msr::tag(), config.msrPresetName(), Chrono::steadyMSecs() - ts);
    }
    else {
        LOG_ERR("%s " RED_BOLD("MSR MODU UYGULANAMADI, HASH ORANI DÜŞÜK OLACAK"), Msr::tag());
    }
...
void xmrig::RxMsr::destroy()
{
    ...
    if (!wrmsr(items, std::vector<CpuThread>(), m_cacheQoS, false)) {
        LOG_ERR("%s " RED_BOLD("BAŞLANGIÇ DURUMU GERi YÜKLENEMEDi" BLACK_BOLD(" (%" PRIu64 " ms)")), Msr::tag(), Chrono::steadyMSecs() - ts);
    }
}
