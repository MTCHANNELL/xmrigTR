...
public:
    ...
    inline void initDataset(uint32_t threads, int priority)
    {
        ...
        if (m_ready) {
            LOG_INFO("%s" GREEN_BOLD("VERi SETi HAZIR:") BLACK_BOLD(" (%" PRIu64 " ms)"), Tags::randomx(), Chrono::steadyMSecs() - ts);
        }
    }
...
private:
    void printAllocStatus(uint64_t ts)
    {
        if (m_dataset->get() != nullptr) {
            ...
            LOG_INFO("%s" GREEN_BOLD("TAHSiS EDiLEN:") CYAN_BOLD(" %zu MB") BLACK_BOLD(" (%zu+%zu)") " BüYüK SAYFALAR: %s%1.0f%% %u/%u" CLEAR " %sJIT" BLACK_BOLD(" (%" PRIu64 " ms)"),
                     Tags::randomx(),
                     pages.size / oneMiB,
            ...
            LOG_WARN(CLEAR "%s" YELLOW_BOLD_S "RANDOMX VERi KüMESi TAHSiS EDiLEMEDi, YAVAŞ MODA GEÇiLDi" BLACK_BOLD(" (%" PRIu64 " ms)"), Tags::randomx(), Chrono::steadyMSecs() - ts);
        }
    }
