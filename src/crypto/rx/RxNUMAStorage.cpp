...
{
    LOG_WARN("%s" CYAN_BOLD("#%u ") RED_BOLD("ATLANDI") YELLOW(" (%s)"), Tags::randomx(), nodeId, reason);
}


static inline void printDatasetReady(uint32_t nodeId, uint64_t ts)
{
    LOG_INFO("%s" CYAN_BOLD("#%u ") GREEN_BOLD("VERi SETi HAZIR:") BLACK_BOLD(" (%" PRIu64 " ms)"), Tags::randomx(), nodeId, Chrono::steadyMSecs() - ts);
}
...
public:
    ...
    inline bool createDatasets(bool hugePages, bool oneGbPages)
    {
        ...
        if (m_datasets.empty()) {
            ...
            LOG_WARN(CLEAR "%s" YELLOW_BOLD_S "RANDOMX VERi KüMELERi TAHSiS EDiLEMEDi, YAVAŞ MODA GEÇiLDi" BLACK_BOLD(" (%" PRIu64 " ms)"), Tags::randomx(), Chrono::steadyMSecs() - ts);
        }
        else {
            if (m_cache) {
...
private:
    static void allocate(RxNUMAStoragePrivate *d_ptr, uint32_t nodeId, bool hugePages, bool oneGbPages)
    {
        ...
        if (!dataset->get()) {
            printSkipped(nodeId, "VERi KüMESi TAHSiS EDiLEMEDi");
    ...
    static void allocateCache(RxNUMAStoragePrivate *d_ptr, uint32_t nodeId, bool hugePages)
    {
        ...
        if (!cache->get()) {
            ...
            LOG_INFO("%s" RED_BOLD("RANDOMX BELLEĞi TAHSiS EDiLEMEDi") BLACK_BOLD(" (%" PRIu64 " ms)"), Tags::randomx(), Chrono::steadyMSecs() - ts);
    ...
    static void printAllocStatus(RxDataset *dataset, uint32_t nodeId, uint64_t ts)
    {
        ...
        LOG_INFO("%s" CYAN_BOLD("#%u ") GREEN_BOLD("TAHSiS EDiLEN") CYAN_BOLD(" %zu MB") " BüYüK SAYFALAR %s%3.0f%%" CLEAR BLACK_BOLD(" (%" PRIu64 " ms)"),
                 Tags::randomx(),
                 nodeId,
                 pages.size / oneMiB,
    ...
    static void printAllocStatus(RxCache *cache, uint32_t nodeId, uint64_t ts)
    {
        ...
        LOG_INFO("%s" CYAN_BOLD("#%u ") GREEN_BOLD("TAHSiS EDiLEN") CYAN_BOLD(" %4zu MB") " BüYüK SAYFALAR %s%3.0f%%" CLEAR " %sJIT" BLACK_BOLD(" (%" PRIu64 " ms)"),
                 Tags::randomx(),
                 nodeId,
                 cache->size() / oneMiB,
                 (pages.isFullyAllocated() ? GREEN_BOLD_S : RED_BOLD_S),
    ...
    void printAllocStatus(uint64_t ts) const
    {
        ...
        LOG_INFO("%s" CYAN_BOLD("-- ") GREEN_BOLD("TAHSiS EDiLEN") CYAN_BOLD(" %4zu MB") " BüYüK SAYFALAR %s%3.0f%% %u/%u" CLEAR BLACK_BOLD(" (%" PRIu64 " ms)"),
                 Tags::randomx(),
                 pages.size / oneMiB,
