    ...
    bool uninstall()
    {
        ...
        if (!reuse) {
            ...
            if (!DeleteService(service)) {
                LOG_ERR("%s " RED("WiNRiNG0 SüRüCüSü KALDIRILAMADI, HATA %u"), Msr::tag(), GetLastError());
                result = false;
            }
        }
...
xmrig::Msr::Msr() : d_ptr(new MsrPrivate())
{
    ...
    if (!d_ptr->manager) {
        if ((err = GetLastError()) == ERROR_ACCESS_DENIED) {
            LOG_WARN("%s " YELLOW_BOLD("MSR KAYITLARINA ERiŞMEK iÇiN YöNETiCi AYRICALIKLARI GEREKLiDiR."), tag());
        }
        else {
            LOG_ERR("%s " RED("HiZMET KONTROL YöNETiCiSi AÇILAMADI, HATA %u"), tag(), err);
        }
    ...
    if (err != ERROR_SUCCESS) {
        LOG_ERR("%s " RED("SüRüCüYE GiDEN YOL ALINAMADI, HATA %u"), tag(), err);
        return;
    }
    ...
    if (d_ptr->service) {
        LOG_WARN("%s " YELLOW("HiZMET ") YELLOW_BOLD("WinRing0_1_2_0") YELLOW(" ZATEN VAR"), tag());
        ...
        if (rc) {
            ...
            if (GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
                ...
                if (QueryServiceConfigA(d_ptr->service, config, buffer.size(), &dwBytesNeeded)) {
                    LOG_INFO("%s " YELLOW("SERViS YOLU: ") YELLOW_BOLD("\"%s\""), tag(), config->lpBinaryPathName);
                }
            }
        }
    ...
    if (d_ptr->driver != INVALID_HANDLE_VALUE) {
        LOG_WARN("%s " YELLOW("HiZMET ") YELLOW_BOLD("WinRing0_1_2_0") YELLOW(" ZATEN VAR, ANCAK FARKLI BiR HiZMET ADIYLA"), tag());
        d_ptr->reuse = true;
        return;
    }
    ...
    if (!d_ptr->reuse) {
        ...
        if (!d_ptr->service) {
            LOG_ERR("%s " RED("WiNRiNG0 SüRüCüSü YüKLENEMEDi, HATA %u"), tag(), GetLastError());
        ...
        if (!StartService(d_ptr->service, 0, nullptr)) {
            ...
            if (err != ERROR_SERVICE_ALREADY_RUNNING) {
                if (err == ERROR_FILE_NOT_FOUND) {
                    LOG_ERR("%s " RED("WiNRiNG0 SüRüCüSüNü BAŞLATAMADI: ") RED_BOLD("\"WinRing0x64.sys BULUNAMADI\""), tag());
                }
                else {
                    LOG_ERR("%s " RED("WiNRiNG0 SüRüCüSüNü BAŞLATAMADI, HATA %u"), tag(), err);
                }
    ...
    d_ptr->driver = CreateFileW(L"\\\\.\\" SERVICE_NAME, GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
    if (d_ptr->driver == INVALID_HANDLE_VALUE) {
        LOG_ERR("%s " RED("WiNRiNG0 SüRüCüSüNE BAĞLANILAMADI, HATA %u"), tag(), GetLastError());;
    }
}
...
bool xmrig::Msr::wrmsr(uint32_t reg, uint64_t value, int32_t cpu)
{
    ...
    static_assert(sizeof(input) == 12, "WiNRiNG0 SüRüCüSü iÇiN GEÇERSiZ YAPI BOYUTU");
