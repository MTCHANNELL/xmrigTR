...
void xmrig::Network::onActive(IStrategy *strategy, IClient *client)
{
    if (m_donate && m_donate == strategy) {
        LOG_NOTICE("%s " WHITE_BOLD("GELiŞTiRiCi BAĞIŞI BAŞLADI"), Tags::network());
        return;
    }
...
#   endif
    ...
    LOG_INFO("%s " WHITE_BOLD("KULLANILAN HAVUZ: %s ") CYAN_BOLD("%s:%d%s ") GREEN_BOLD("%s") " " BLACK_BOLD("%s"),
             Tags::network(), client->mode(), pool.host().data(), pool.port(), zmq_buf, tlsVersion ? tlsVersion : "", client->ip().data());

    const char *fingerprint = client->tlsFingerprint();
    if (fingerprint != nullptr) {
        LOG_INFO("%s " BLACK_BOLD("PARMAK iZi (SHA-256): \"%s\""), Tags::network(), fingerprint);
    }
}
...
{
    if (m_donate && m_donate == strategy) {
        LOG_NOTICE("%s " WHITE_BOLD("GELiŞTiRiCi BAĞIŞI BiTTi"), Tags::network());
        m_strategy->resume();
    }

    if (!m_strategy->isActive()) {
        LOG_ERR("%s " RED("AKTiF HAVUZ YOK, MADENCiLiĞi DURDUR"), Tags::network());
...
void xmrig::Network::onResultAccepted(IStrategy *, IClient *, const SubmitResult &result, const char *error)
{
    ...
    if (error) {
        LOG_INFO("%s " RED_BOLD("REDDEDiLMiŞ") " (%" PRId64 "/%" PRId64 ") FARK: " WHITE_BOLD("%" PRIu64 "%s") " " RED("\"%s\"") " " BLACK_BOLD("(%" PRIu64 " MS)"),
                 backend_tag(result.backend), m_state->accepted(), m_state->rejected(), diff, scale, error, result.elapsed);
    }
    else {
		Log::print(WHITE_BOLD_S "                          ╔══════════════════════════════════════════════════════════════════════════╗");
        LOG_INFO("║%s " GREEN_BOLD("KABUL EDiLMiŞ:") " (%" PRId64 "/%" PRId64 ") FARK: " WHITE_BOLD("%" PRIu64 "%s") " " BLACK_BOLD("(%" PRIu64 " MS)     ║║║║║║║║║║║║║║║"),
...
#   endif
    {
        ...
        char height_buf[64] = {};
        if (job.height() > 0) {
            snprintf(height_buf, sizeof(height_buf), " YüKSEKLiK: " WHITE_BOLD("%" PRIu64), job.height());
        }

        LOG_INFO("%s " MAGENTA_BOLD("YENi iŞ") " GELDi: " WHITE_BOLD("%s:%d%s") " FARK: " WHITE_BOLD("%" PRIu64 "%s") " ALGORiTMA: " WHITE_BOLD("%s") "%s%s",
