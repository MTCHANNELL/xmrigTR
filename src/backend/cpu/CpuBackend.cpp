...
public:
    ...
    {
        if (m_started == 0) {
            LOG_ERR("%s " RED_BOLD("KAPALI") YELLOW(" (iŞ PARÇACIKLARI BAŞLATILAMADI)"), Tags::cpu());

            return;
        }

        LOG_INFO("%s" GREEN_BOLD(" HAZIR") " iŞ PARÇACIKLARI: %s%zu/%zu (%zu)" CLEAR " BüYüK SAYFALAR: %s%1.0f%% %zu/%zu" CLEAR " HAFIZA: " CYAN_BOLD("%zu KB") BLACK_BOLD(" (%" PRIu64 " ms)"),
                 Tags::cpu(),
...
public:
    ...
    inline void start()
    {
        LOG_INFO("%s KULLANILAN PROFiL: " BLUE_BG(WHITE_BOLD_S " %s ") WHITE_BOLD_S " (" CYAN_BOLD("%zu") WHITE_BOLD(" :iŞ PARÇACIĞI%s)") " ÇALIŞMA DEFTERi: " CYAN_BOLD("%zu KB"),
                 Tags::cpu(),
                 profileName.data(),
                 threads.size(),
                 threads.size() > 1 ? "s" : "",
...
#   ifdef XMRIG_ALGO_ARGON2
    ...
    if ((f == Algorithm::ARGON2) || (f == Algorithm::RANDOM_X)) {
        if (argon2::Impl::select(d_ptr->controller->config()->cpu().argon2Impl())) {
            LOG_INFO("%s UYGULAMA OLARAK " WHITE_BOLD("-argon2-") " KULLANILIYOR " CSI "1;%dm" "%s",
                     Tags::cpu(),
...
void xmrig::CpuBackend::printHashrate(bool details)
{
	...
	Log::print(WHITE_BOLD_S "╔════════════════════════════════════════════════════════════════════════════════════════════════╗");
    Log::print(WHITE_BOLD_S "║ MMMM    MMMM MMMMMMMMMMM    MMMMMM  MMM  MMM      MMM     MMM   MMM  MMM   MMM MMMMMMM MMM     ║");      
    Log::print(WHITE_BOLD_S "║ MMMM    MMMM MMMMMMMMMMM  MMMMMMMMM MMM  MMM     MM MM    MMMM  MMM  MMMM  MMM MMMMMMM MMM     ║");      
    Log::print(WHITE_BOLD_S "║ MMMMM  MMMMM     MMM     MMMMM  MMM MMM  MMM    MM   MM   MMMMMMMMM  MMMMMMMMM MMM     MMM     ║");      
    Log::print(WHITE_BOLD_S "║ MMMMMMMMMMMM     MMM     MMMM       MMMMMMMM   MMM   MMM  MMMMMMMMM  MMMMMMMMM MMMMMMM MMM     ║");      
    Log::print(WHITE_BOLD_S "║ MMM MMMM MMM     MMM     MMMMM  MMM MMM  MMM  MMMMMMMMMMM MMM MMMMM  MMM MMMMM MMM     MMM     ║");      
    Log::print(WHITE_BOLD_S "║ MMM  MM  MMM     MMM      MMMMMMMMM MMM  MMM  MMMMMMMMMMM MMM  MMMM  MMM  MMMM MMMMMMM MMMMMMM ║"); 
    Log::print(WHITE_BOLD_S "║ MMM  MM  MMM     MMM        MMMMMM  MMM  MMM  MMMM   MMMM MMM   MMM  MMM   MMM MMMMMMM MMMMMMM ║"); 
	Log::print(WHITE_BOLD_S "╚════════════════════════════════════════════════════════════════════════════════════════════════╝");
	Log::print(WHITE_BOLD_S "╔═══════════════════════════════════════════════════╗");
    Log::print(WHITE_BOLD_S "║  iŞLEMCi ║ YAKINLIK ║10 SN H/S║60 SN H/S║15 DK H/S║");
	Log::print(WHITE_BOLD_S "║═══════════════════════════════════════════════════║");
    size_t i = 0;
...
void xmrig::CpuBackend::setJob(const Job &job)
{
    ...
    if (d_ptr->profileName.isNull() || threads.empty()) {
        LOG_WARN("%s " RED_BOLD("KAPALI") YELLOW(" (UYGUN YAPILANDIRMA BULUNAMADI)"), Tags::cpu());
...
void xmrig::CpuBackend::stop()
{
    ...
    LOG_INFO("%s" YELLOW(" DURDU") BLACK_BOLD(" (%" PRIu64 " ms)"), Tags::cpu(), Chrono::steadyMSecs() - ts);
}
