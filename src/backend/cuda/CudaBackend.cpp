...
static const char *kLabel       = "NViDiA EKRAN KARTI:";
...
{
    Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-13s") RED_BOLD("KAPALI") "%s", label, reason);
}
...
public:
    ...
    {
        if (m_started == 0) {
            LOG_ERR("%s " RED_BOLD("KAPALI") YELLOW(" (iŞ PARÇACIKLARI BAŞLATILAMADI)"), Tags::nvidia());

            return;
        }

        LOG_INFO("%s" GREEN_BOLD(" HAZIR") " threads " "%s%zu/%zu" BLACK_BOLD(" (%" PRIu64 " ms)"),
                 Tags::nvidia(),
...
public:
    ...
    void init(const CudaConfig &cuda)
    {
        ...
        if (!CudaLib::init(cuda.loader())) {
            Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-13s") RED_BOLD("KAPALI ") RED("(%s)"), kLabel, CudaLib::lastError());
        ...
        if (!runtimeVersion || !driverVersion || !CudaLib::deviceCount()) {
            return printDisabled(kLabel, RED_S " (CiHAZ YOK)");
        }
...
#       ifdef XMRIG_FEATURE_NVML
        if (cuda.isNvmlEnabled()) {
            if (NvmlLib::init(cuda.nvmlLoader())) {
                ...
                printDisabled(kNvmlLabel, RED_S " (NVML YüKLENEMEDi)");
            }
        }
        else {
...
void xmrig::CudaBackend::setJob(const Job &job)
{
    ...
    if (d_ptr->profileName.isNull() || threads.empty()) {
        LOG_WARN("%s " RED_BOLD("KAPALI") YELLOW(" (UYGUN YAPILANDIRMA BULUNAMADI)"), Tags::nvidia());
