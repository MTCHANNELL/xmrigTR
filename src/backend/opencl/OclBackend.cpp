...
static const char *kLabel       = "AMD EKRAN KARTI   :";
...
public:
    ...
    {
        if (m_started == 0) {
            LOG_ERR("%s " RED_BOLD("KAPALI") YELLOW(" (iŞ PARÇACIKLARI BAŞLATILAMADI)"), Tags::opencl());
...
#       endif
        ...
        Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-13s") CYAN_BOLD("#%zu ") WHITE_BOLD("%s") "/" WHITE_BOLD("%s"), "AMD EKRAN KARTI   :", platform.index(), platform.name().data(), platform.version().data());
...
void xmrig::OclBackend::setJob(const Job &job)
{
    ...
    if (d_ptr->profileName.isNull() || threads.empty()) {
        LOG_WARN("%s " RED_BOLD("KAPALI") YELLOW(" (UYGUN YAPILANDIRMA BULUNAMADI)"), Tags::opencl());
