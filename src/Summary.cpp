...
#ifdef XMRIG_OS_WIN
static constexpr const char *kHugepagesSupported = GREEN_BOLD("iZiN VERiLDi");
#else
static constexpr const char *kHugepagesSupported = GREEN_BOLD("DESTEKLENiYOR");
#endif
...
static void print_pages(const Config *config)
{
    Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-13s") "%s",
               "BüYüK SAYFALAR    :", config->cpu().isHugePages() ? (VirtualMemory::isHugepagesAvailable() ? kHugepagesSupported : RED_BOLD("KULLANIM DIŞI")) : RED_BOLD("KAPALI"));

#   ifdef XMRIG_ALGO_RANDOMX
#   ifdef XMRIG_OS_LINUX
    Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-13s") "%s",
               "1GB SAYFALAR      :", (VirtualMemory::isOneGbPagesAvailable() ? (config->rx().isOneGbPages() ? kHugepagesSupported : YELLOW_BOLD("KAPALI")) : YELLOW_BOLD("KULLANIM DIŞI")));
#   else
    Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-13s") "%s", "1GB SAYFALAR      :", YELLOW_BOLD("KULLANIM DIŞI"));
#   endif
#   endif
}
...
#   if defined(XMRIG_FEATURE_HWLOC)
    Log::print(WHITE_BOLD("   %-13s") BLACK_BOLD("     :L2 öNBELLEK:") WHITE_BOLD("%.1f MB") BLACK_BOLD(" - L3 öNBELLEK:") WHITE_BOLD("%.1f MB")
...
#   else
    Log::print(WHITE_BOLD("   %-13s") BLACK_BOLD("iŞ PARÇACIĞI:") CYAN_BOLD("%zu"), "", info->threads());
#   endif
}
...
#   ifdef XMRIG_FEATURE_DMI
    ...
    for (const auto &memory : reader.memory()) {
        ...
        if (memory.size()) {
            ...
            Log::print(WHITE_BOLD("   %-13s") "%s: " BLACK_BOLD("<SLOT BOŞ>"), "", memory.id().data());
        }
    }
    ...
    if (board.isValid()) {
        Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-13s") WHITE_BOLD("%s") " - " WHITE_BOLD("%s"), "ANAKART           :", board.vendor().data(), board.product().data());
    }
#   endif
}
...
#   ifdef XMRIG_FEATURE_ASM
    if (config->cpu().assembly() == Assembly::AUTO) {
        ...
        Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-13sOTOMATiK SEÇiLEN:%s"), "iŞLEMCi TüRü      :", asmName(assembly));
    }
    else {
        Log::print(GREEN_BOLD(" * ") WHITE_BOLD("%-13s%s"), "iŞLEMCi TüRü      :", asmName(config->cpu().assembly()));
    }
#   endif
}
...
{
    if (Log::isColors()) {
        Log::print(GREEN_BOLD(" * ") WHITE_BOLD("KOMUTLAR          :") MAGENTA_BG_BOLD("H") WHITE_BOLD(":HASH ORANI, ")
                                                                 MAGENTA_BG_BOLD("P") WHITE_BOLD(":DURAKLAT, ")
                                                                 MAGENTA_BG_BOLD("R") WHITE_BOLD(":DEVAM ET, ")
                                                                 MAGENTA_BG(WHITE_BOLD_S "S") WHITE_BOLD(":SONUÇLAR, ")
                                                                 MAGENTA_BG_BOLD("C") WHITE_BOLD(":BAĞLANTI")
                   );
    }
    else {
        Log::print(" * KOMUTLAR          :'h' :HASH ORANI, 'p' :DURAKLAT, 'r' :DEVAM ET, 's' :SONUÇLAR, 'c' :BAĞLANTI");
    }
}
